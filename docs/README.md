# slink Documentation

`slink` is a secure SSH connection manager that encrypts your credentials locally and lets you connect with a single command.

## Documentation Index

- **Getting Started**
  - [Installation](getting-started/installation.md)
  - [Quick Start](getting-started/quickstart.md)

- **User Guide**
  - [Command Reference](user-guide/commands.md)
  - [GUI Mode](user-guide/gui.md)
  - [Multi-Login Engine](user-guide/ml.md)
  - [Groups & Workspaces](user-guide/groups-workspaces.md)

- **Configuration**
  - [Config Files](configuration/config-files.md)
  - [SSH Config Import](configuration/ssh-config-import.md)

- **Security**
  - [Encryption](security/encryption.md)
  - [File Permissions](security/file-permissions.md)

- **Development**
  - [Architecture](development/architecture.md)
  - [Building & Packaging](development/building.md)

- **Reference**
  - [Changelog](reference/changelog.md)
  - [License](reference/license.md)
